# Supabase Setup Guide — Noctra SEO

Este documento contiene las instrucciones para configurar tu base de datos. 

> [!CAUTION]
> **NO COPIES TODO ESTE ARCHIVO MD.** 
> Pegar texto markdown en el editor de Supabase causará errores de sintaxis (como el error `#` que viste).

### Opción 1: Archivo SQL Puro (Recomendado)
Abre el archivo [supabase/setup.sql](file:///Users/manu/Documents/1.Projects/Noctra-studio/website/noctra-seo/supabase/setup.sql), copia **absolutamente todo** su contenido y pégalo en el SQL Editor. Ese archivo no tiene texto markdown y funcionará directamente.

### Opción 2: Copiar el bloque de abajo
Si prefieres copiar desde aquí, asegúrate de copiar **SOLO** lo que está dentro del bloque de código gris (sin los títulos de arriba):
...
create policy "org_isolation" on organizations for all using (id = current_user_org_id());
create policy "org_isolation" on users for all using (org_id = current_user_org_id());
create policy "org_isolation" on projects for all using (org_id = current_user_org_id());
create policy "org_isolation" on domains for all using (project_id in (select id from projects where org_id = current_user_org_id()));

-- Tracker Permissions
create policy "tracker_insert" on pageviews for insert with check (true);
create policy "tracker_insert" on page_seo_signals for insert with check (true);
create policy "tracker_update" on page_seo_signals for update using (true);
create policy "tracker_insert" on web_vitals for insert with check (true);

-- Domain data access
create policy "org_isolation_pageviews" on pageviews for select using (domain_id in (select d.id from domains d join projects p on d.project_id = p.id where p.org_id = current_user_org_id()));
create policy "org_isolation_seo" on page_seo_signals for select using (domain_id in (select d.id from domains d join projects p on d.project_id = p.id where p.org_id = current_user_org_id()));
create policy "org_isolation_vitals" on web_vitals for select using (domain_id in (select d.id from domains d join projects p on d.project_id = p.id where p.org_id = current_user_org_id()));
create policy "org_isolation_alerts" on alerts for all using (domain_id in (select d.id from domains d join projects p on d.project_id = p.id where p.org_id = current_user_org_id()));
create policy "org_isolation_gsc" on gsc_snapshots for all using (domain_id in (select d.id from domains d join projects p on d.project_id = p.id where p.org_id = current_user_org_id()));
create policy "org_isolation_geo" on geo_checks for all using (domain_id in (select d.id from domains d join projects p on d.project_id = p.id where p.org_id = current_user_org_id()));
create policy "org_isolation_audit" on audit_jobs for all using (domain_id in (select d.id from domains d join projects p on d.project_id = p.id where p.org_id = current_user_org_id()));
```
