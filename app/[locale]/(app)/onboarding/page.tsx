    ...
    <div className="space-y-2">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-[#14141C] border border-[#1E1E2A] rounded-lg px-3 py-2 text-sm text-[#F1F1F5] placeholder:text-[#8B8B9A] focus:outline-none focus:border-[#6366F1] transition-colors"
        />
        <button
          onClick={add}
          className="px-3 py-2 bg-[#1E1E2A] rounded-lg text-sm text-[#8B8B9A] hover:text-[#F1F1F5] transition-colors"
        >
          +
        </button>
      </div>
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {value.map(tag => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 px-2 py-0.5 bg-[#6366F115] border border-[#6366F130] text-[#6366F1] text-xs rounded-md"
            >
              {tag}
              <button
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      <label className="text-sm font-medium text-[#C5C5D0]">{label}</label>
      {children}
    </div>
  );
}
            ...
            >
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        aiContext.business_type === id
                          ? 'bg-[#6366F115] border-[#6366F1] text-[#6366F1]'
              ...
              <Field label="¿En qué industria o nicho operas?">
                <input
                  ...
                  className="w-full bg-[#14141C] border border-[#1E1E2A] rounded-lg px-3 py-2 text-sm text-[#F1F1F5] placeholder:text-[#8B8B9A] focus:outline-none focus:border-[#6366F1] transition-colors"
                />
              </Field>
            ...
            >
              <Field label="Describe brevemente a tu cliente ideal">
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-[#14141C] border border-[#1E1E2A] rounded-lg px-3 py-2 text-sm text-[#F1F1F5] placeholder:text-[#8B8B9A] focus:outline-none focus:border-[#6366F1] transition-colors resize-none"
                />
              </Field>
            ...
            >
              <Field label="¿Cómo quieres recibir alertas?">
                <div className="space-y-2">
                  {[
                    ...
                    <div key={key} className="flex items-center justify-between p-3 bg-[#14141C] border border-[#1E1E2A] rounded-lg">
                      <span className="text-sm text-[#F1F1F5]">{label}</span>
                      <button
                      ...
                      >
                        <span className={cn(
                          ...
                          notifPrefs[key as keyof NotificationPrefs] ? 'translate-x-4' : 'translate-x-0.5'
                        )} />
                      </button>
                    </div>
                  ))}
              ...
              <Field label="¿A qué hora quieres el digest diario?">
                <input
                  ...
                  className="bg-[#14141C] border border-[#1E1E2A] rounded-lg px-3 py-2 text-sm text-[#F1F1F5] focus:outline-none focus:border-[#6366F1] transition-colors"
                />
              </Field>
          ...
          {step === 5 && (
            ...
            >
              ...
              <div className="flex items-center gap-2 p-3 bg-[#14141C] border border-[#1E1E2A] rounded-lg">
                <span className="text-xs text-[#8B8B9A]">Tu Site ID:</span>
                <code className="flex-1 font-mono text-sm text-[#6366F1]">{siteId || 'Cargando...'}</code>
                <button
                  onClick={() => copy(siteId, 'siteid')}
                  className="flex items-center gap-1 text-xs text-[#8B8B9A] hover:text-[#F1F1F5] transition-colors"
                >
                  {copied === 'siteid' ? <Check size={13} className="text-[#10B981]" /> : <Copy size={13} />}
                </button>
              </div>

              {/* Script snippet */}
              <div>
                <p className="text-xs text-[#8B8B9A] mb-2 font-medium">Opción A — Script universal:</p>
                <div className="relative group">
                  <pre className="bg-[#0A0A0F] border border-[#1E1E2A] rounded-lg p-4 text-xs font-mono text-[#10B981] overflow-x-auto">
                    {scriptSnippet}
                  </pre>
                  <button
              ...
              {/* WordPress option */}
              <div className="p-3 bg-[#14141C] border border-[#1E1E2A] rounded-lg text-sm text-[#8B8B9A]">
                <p className="font-medium text-[#C5C5D0] mb-1">Opción B — WordPress:</p>
                Instala el plugin <span className="text-[#6366F1]">Noctra SEO Tracker</span> y añade tu Site ID en la configuración.
              </div>
